...
static torch::Tensor fp8_fp4_paged_mqa_logits(const std::tuple<torch::Tensor, std::optional<torch::Tensor>>& q,
    ...
    const int split_kv = (arch_major == 12) ? 128 : 256;  // SM120: 2 groups x 64
    // Logits row stride must be 1024-byte aligned
    const int stride_logits_alignment = 1024 / static_cast<int>(c10::elementSize(logits_dtype));
...
// The source tree builds this header through the TVM-FFI API.  Keep the
// legacy pybind registration available for the old extension build without
// requiring pybind types in the TVM-FFI translation unit.
#if 0
